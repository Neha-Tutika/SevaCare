...
Expected JSON format:
{
  ...
  "simple_explanation": "A simple, easy-to-understand summary of how to take the medicines in English. E.g., 'Take one blue pill after breakfast and one white pill before sleeping.'",
  "translated_explanation": "The same simple summary translated into the requested language ({language})."
}
...
async def extract_prescription_data(file: UploadFile, language: str = "Hindi") -> Dict[str, Any]:
    """
    ...
    if not GEMINI_API_KEY:
        ...
        return {
            ...
            "simple_explanation": "Take Paracetamol twice a day (morning and night), and Vitamin D3 once in the morning.",
            "translated_explanation": f"(Mock Translation to {language}) Take Paracetamol twice a day..."
        }
    ...
    try:
        ...
        if file.filename.lower().endswith((".png", ".jpg", ".jpeg")):
            ...
            prompt = EXTRACTION_PROMPT.replace("{language}", language)
...
def normalize_extracted_data(data: Dict[str, Any]) -> Dict[str, Any]:
    ...
    normalized = {
        ...
        "simple_explanation": data.get("simple_explanation", "No explanation available."),
        "translated_explanation": data.get("translated_explanation", "अनुवाद उपलब्ध नहीं है."),
        "drugs": [],
    }

    # Validate doctor name
    if not normalized["doctor_name"]:
        print("Warning: Doctor name not found, using default.")
        normalized["doctor_name"] = "Unknown Doctor"
    ...
    if not normalized["drugs"]:
        print("Warning: No valid medications found.")
        # Don't raise error, just return empty list or what we have
